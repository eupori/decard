...
### UX 마무리
- [x] 빈칸형 카드의 빈칸 스타일링 (`_____` → 하이라이트 위젯)
- [ ] 토스트/알림 일관성 (삭제, 채택 등)
- [ ] 모바일 터치 UX 세밀 확인 (버튼 크기, 스와이프)
- [x] 앱 아이콘 커스텀 (민트+네이비 테마 아이콘)

### 배포
- [x] 백엔드: EC2 Docker Compose 배포 (기존 인프라 활용)
- [x] 프론트: nginx 웹 배포 (decard.eupori.dev)
- [x] 도메인 설정 (decard.eupori.dev + decard-api.eupori.dev)
- [x] HTTPS 확인
- [x] API URL을 프로덕션으로 변경 (환경변수화)
...
- [x] 카카오 OAuth 로그인 (REST API 직접, JWT 7일)
- [x] 로그인 화면 (카카오 동작 + Google/Apple/이메일 목업)
- [x] 듀얼 인증 (JWT user_id 우선, device_id 폴백)
- [x] 디바이스 세션 마이그레이션 (로그인 시 자동)
- [x] 프로필 닉네임 표시
- [x] 프로덕션 배포

---

## Phase 2.5: 백그라운드 생성 + UX 개선 ✅ 완료

### 백그라운드 카드 생성
- [x] POST /generate 즉시 반환 (status="processing")
- [x] asyncio.create_task 백그라운드 생성 + 별도 DB 세션
- [x] 세션 목록에 processing/failed 상태 표시
- [x] 실패 시 status="failed" + 에러 로깅

### 프론트엔드 폴링 + UX
- [x] 홈 화면 10초 폴링 (processing 세션 감지)
- [x] 포그라운드 대기 모드 (원형 프로그레스 + 예상 시간)
- [x] 백그라운드 전환 모드 (홈에서 폴링 대기)
- [x] 재치 있는 상태 메시지 로테이션
- [x] 완료 시 스낵바 알림 + 세션 목록 갱신
- [x] processing 세션 탭 시 안내 토스트
...
| 기능 | 이유 | 시기 |
|------|------|------|
| ~~로그인/회원가입~~ | ~~검증 단계에서 불필요~~ | ✅ 완료 (카카오 OAuth) |
...
## Phase 5.5: UX 개선 체크리스트

### 학습 화면 (StudyScreen)
- [x] 답변 폰트 크기 확대 (titleMedium → titleLarge)
- [x] 질문 반응형 텍스트 크기 (길이별 headlineSmall/titleLarge/titleMedium)
- [x] 답변 자동 포매팅 (온점/번호/단계/항목 기호 줄바꿈)
- [x] 한글 단어 기준 줄바꿈 (Word Joiner)
- [x] 긴 답변 좌측 정렬, 짧은 답변 중앙 정렬
- [x] 근거를 카드 하단에 인라인 표시
- [x] 힌트 텍스트 처음 2장만 표시
- [x] 진행률 퍼센트 표시
- [x] 셔플 확인 다이얼로그 (2장째부터)
- [ ] 완료 화면에 "순서 유지" 재학습 옵션 추가
- [ ] 극단적 긴 텍스트(200자+) 최소 폰트 크기 보장

### 검수 화면 (ReviewScreen)
- [ ] 카드 편집 중 취소 시 "저장하지 않고 나가시겠어요?" 확인
- [ ] 학습하기 0장일 때 "먼저 카드를 채택해주세요" 안내
- [ ] 다크모드 pendingColor 가독성 확인

### 홈 화면 (HomeScreen)
- [ ] API 오류 코드별 한국어 메시지 (401/413/네트워크 등)
- [ ] 업로드 실패 시 재시도 버튼
- [ ] PDF 선택 후 파일 크기/페이지 수 미리보기

### 공통
- [ ] 색상 대비 WCAG AA 검증 (특히 다크모드 보조 텍스트)
- [ ] 폴더 편집 다이얼로그 공통 위젯 추출
- [ ] 세션 리스트 아이템 공통 위젯 추출
- [ ] Semantics 레이블 추가 (접근성)
